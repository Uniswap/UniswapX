...
import {IProtocolFeeController, IProtocolFeeControllerV2} from "../interfaces/IProtocolFeeController.sol";
import {CurrencyLibrary} from "../lib/CurrencyLibrary.sol";
import {ResolvedOrder, ResolvedOrderV2, OutputToken} from "../base/ReactorStructs.sol";
...
abstract contract ProtocolFeesV2 is Owned {
    using SafeTransferLib for ERC20;
    using FixedPointMathLib for uint256;
    using CurrencyLibrary for address;
    ...
    /// @dev The address of the fee controller
    IProtocolFeeControllerV2 public feeController;
    ...
    /// @dev modifies the orders to include protocol fee outputs
    /// @param order The encoded order to inject fees into
    function _injectFees(ResolvedOrderV2 memory order) internal view {
        if (address(feeController) == address(0)) {
            return;
        }
    ...
    function setProtocolFeeController(address _newFeeController) external onlyOwner {
        address oldFeeController = address(feeController);
        feeController = IProtocolFeeControllerV2(_newFeeController);
        emit ProtocolFeeControllerSet(oldFeeController, _newFeeController);
    }
}
