...
import {ResolvedOrderV2} from "./ReactorStructs.sol";
...
/// @dev All hooks MUST inherit from this contract to ensure token transfers happen
abstract contract BaseHook is IPreExecutionHook {
    ...
    function _beforeTokenTransfer(address filler, ResolvedOrderV2 calldata resolvedOrder) internal virtual {
    ...
    function _afterTokenTransfer(address filler, ResolvedOrderV2 calldata resolvedOrder) internal virtual {
        // Default implementation: no-op
        // Derived contracts can override to add custom logic
    }
    ...
    function _transferInputTokens(ResolvedOrderV2 calldata order, address to) private {
