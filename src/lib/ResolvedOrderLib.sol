...
library ResolvedOrderLib {
    ...
    function prepare(ResolvedOrder memory resolvedOrder, address filler) internal view returns (ResolvedOrder memory) {
        ...
        if (resolvedOrder.info.preparationContract != address(0)) {
            return IOrderPreparation(resolvedOrder.info.preparationContract).prepare(resolvedOrder, filler);
        }
