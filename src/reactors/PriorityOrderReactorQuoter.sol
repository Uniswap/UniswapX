...
contract PriorityOrderReactorQuoter is BaseReactor {
    ...
    mapping(bytes32 => uint256) public orderAuctionStartBlocks;
        ...
        returns (ResolvedOrder memory resolvedOrder)
    {
        ...
        resolvedOrder = ResolvedOrder({
            ...
            sig: abi.encode(signedOrder.sig, auctionStartBlock),
            hash: orderHash
        });
    }
    ...
    function _transferInputTokens(ResolvedOrder memory order, address to) internal override {
        (bytes memory sig, uint256 auctionStartBlock) = abi.decode(order.sig, (bytes, uint256));
