...
interface IDCARegistry {
    ...
    event DCAIntentUpdated(
        bytes32 indexed intentHash,
        address indexed owner,
        DCAIntentUpdate update
    );
    ...
    event DCAChunkExecuted(
        ...
        uint256 executionPrice,  // Actual price achieved (output/input * 1e18)
        uint256 timestamp
    );
    ...
    function getExecutionState(bytes32 intentHash) 
        external 
        view 
        returns (DCAExecutionState memory state);

    /// @notice Get the intent owner
    /// @param intentHash Hash of the DCA intent
    /// @return owner The owner address
    function getIntentOwner(bytes32 intentHash) 
        external 
        view 
    ...
    function isOrderNonceUsed(bytes32 intentHash, bytes32 orderNonce) 
        external 
        view 
        returns (bool used);
    ...
    function getActiveIntentsForOwner(address owner)
        external
        view
        returns (bytes32[] memory intentHashes);
