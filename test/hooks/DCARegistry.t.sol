...
import {IPreExecutionHookV2} from "../../src/interfaces/IPreExecutionHookV2.sol";
...
import {PriorityAuctionResolverV2} from "../../src/resolvers/PriorityAuctionResolverV2.sol";
...
contract DCARegistryTest is Test, DCAIntentSignature, PermitSignature, DeployPermit2 {
    ...
    PriorityAuctionResolverV2 resolver;
    MockERC20 tokenIn;
    MockERC20 tokenOut;
    IPermit2 permit2;
    ...
    function setUp() public {
        ...
        resolver = new PriorityAuctionResolverV2(permit2);
    ...
    function test_EIP1271_isValidSignature_afterPreExecutionHook() public {
        ...
        ResolvedOrderV2 memory order = ResolvedOrderV2({
            info: OrderInfoV2({
                ...
                preExecutionHook: IPreExecutionHookV2(address(dcaRegistry)),
        ...
        dcaRegistry.preExecutionHookV2(filler, order);
    ...
    function test_preExecutionHook_pullsTokensFromSwapper() public {
        ...
        assertEq(tokenIn.balanceOf(address(dcaRegistry)), registryBalanceBefore + 500e18, "Registry balance should increase");
    }
    ...
    function test_rejectUsedOrderNonce() public {
        ...
        IDCARegistry.DCAOrderCosignerData memory cosignerData1 = createBasicCosignerData(
            swapper,
            500e18,
            450e18,
            orderNonce
        ...
        vm.prank(address(reactor));
        dcaRegistry.preExecutionHookV2(filler, order1);
        ...
        dcaRegistry.preExecutionHookV2(filler, order2);
    }
    ...
    function testFuzz_preExecutionHook_differentAmounts(uint256 inputAmount) public {
        ...
        order.outputs[0] = OutputToken({token: address(tokenOut), amount: inputAmount * 90 / 100, recipient: address(this)});
    ...
    function _createAndExecuteOrder(uint256 orderNumber, bytes32 orderNonce) internal returns (bytes32 orderHash) {
        ...
        IDCARegistry.DCAIntent memory intent = createPublicDCAIntent(
            ...
            address(tokenOut),
            cosigner,
            keccak256(abi.encode("private params", orderNumber))
        );
