...
function hashAuthorization(parameters) {
  const { chainId: chainId2, contractAddress, nonce, to } = parameters;
...
async function experimental_signAuthorization(parameters) {
  const { contractAddress, chainId: chainId2, nonce, privateKey: privateKey2, to = "object" } = parameters;
  const signature = await sign({
    hash: hashAuthorization({ contractAddress, chainId: chainId2, nonce }),
...
function serializeAuthorizationList(authorizationList) {
  ...
  for (const authorization of authorizationList) {
    const { contractAddress, chainId: chainId2, nonce, ...signature } = authorization;
    serializedAuthorizationList.push([
      chainId2 ? toHex(chainId2) : "0x",
      contractAddress,
...
function assertTransactionEIP7702(transaction) {
  ...
  if (authorizationList) {
    for (const authorization of authorizationList) {
      const { contractAddress, chainId: chainId2 } = authorization;
...
function assertTransactionEIP1559(transaction) {
  const { chainId: chainId2, maxPriorityFeePerGas, maxFeePerGas, to } = transaction;
  if (chainId2 <= 0)
    throw new InvalidChainIdError({ chainId: chainId2 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
function assertTransactionEIP2930(transaction) {
  const { chainId: chainId2, maxPriorityFeePerGas, gasPrice, maxFeePerGas, to } = transaction;
  if (chainId2 <= 0)
    throw new InvalidChainIdError({ chainId: chainId2 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
function serializeTransactionEIP2930(transaction, signature) {
  const { chainId: chainId2, gas, data, nonce, to, value, accessList, gasPrice } = transaction;
...
function serializeTransactionLegacy(transaction, signature) {
  const { chainId: chainId2 = 0, gas, data, nonce, to, value, gasPrice } = transaction;
  ...
  if (signature) {
    const v = (() => {
      ...
      if (chainId2 > 0)
        return BigInt(chainId2 * 2) + BigInt(35n + signature.v - 27n);
...
function hashFeedTemplate(template) {
  return keccak256(
    encodeAbiParameters(
      [
        { type: "bytes32" },
