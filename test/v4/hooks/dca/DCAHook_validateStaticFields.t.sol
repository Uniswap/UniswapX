...
contract DCAHook_validateStaticFieldsTest is Test, DeployPermit2 {
    ...
    function test_validateStaticFields_revert_swapperMismatch() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDCAHook.SwapperMismatch.selector,
                wrongSwapper,
                SWAPPER
            )
        );
    ...
    function test_validateStaticFields_revert_wrongInputToken() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDCAHook.WrongInputToken.selector,
                address(WRONG_INPUT_TOKEN),
                address(INPUT_TOKEN)
            )
        );
        hook.validateStaticFields(intent, order);
    ...
    function test_validateStaticFields_revert_wrongOutputToken_singleOutput() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IDCAHook.WrongOutputToken.selector,
                address(WRONG_OUTPUT_TOKEN),
                address(OUTPUT_TOKEN)
            )
        );
        hook.validateStaticFields(intent, order);
    ...
    function testFuzz_validateStaticFields_differentChainIds(
        uint256 fuzzChainId
    ) public {
    ...
    function testFuzz_validateStaticFields_multipleOutputs(
        uint8 outputCount
    ) public view {
