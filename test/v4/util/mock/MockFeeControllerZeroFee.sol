...
/// @notice Mock protocol fee controller that returns a fee output with token address(0)
contract MockFeeControllerZeroFee is IProtocolFeeController, Owned(msg.sender) {
    ...
    function getFeeOutputs(ResolvedOrder memory order) external view override returns (OutputToken[] memory result) {
        ...
        // Return fee output with address(0) token
        result[0] = OutputToken({token: address(0), amount: feeAmount, recipient: feeRecipient});
    }
