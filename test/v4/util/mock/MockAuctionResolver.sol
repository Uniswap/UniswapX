...
contract MockAuctionResolver is IAuctionResolver {
    ...
    function resolve(SignedOrder calldata signedOrder) external view override returns (ResolvedOrder memory) {
        ...
        return ResolvedOrder({
            ...
            sig: signedOrder.sig,
            hash: mockOrder.witnessHash(address(this)), // Witness hash that includes resolver and full order
            auctionResolver: address(this),
            witnessTypeString: MockOrderLib.PERMIT2_ORDER_TYPE
        });
    }
...
contract MaliciousAuctionResolver is IAuctionResolver {
    ...
    function resolve(SignedOrder calldata signedOrder) external view override returns (ResolvedOrder memory) {
        ...
        return ResolvedOrder({
            ...
            outputs: mockOrder.outputs,
            sig: signedOrder.sig,
            hash: mockOrder.witnessHash(address(this)), // Witness hash (will fail signature verification)
            auctionResolver: address(this),
            witnessTypeString: MockOrderLib.PERMIT2_ORDER_TYPE
        });
    }
