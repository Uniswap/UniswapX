...
import {V3DutchOrder, V3DutchOrderLib, V3DutchOrderReactor, CosignerData} from "../../src/reactors/V3DutchOrderReactor.sol";
import {V3DutchOutput, V3DutchInput} from "../../src/lib/V3DutchOrderLib.sol";
import {PriorityOrder, PriorityOrderLib, PriorityInput, PriorityOutput, PriorityCosignerData} from "../../src/lib/PriorityOrderLib.sol";
...
import {V2DutchOrder, V2DutchOrderLib, V2DutchOrderReactor, CosignerData as V2CosignerData, DutchInput, DutchOutput} from "../../src/reactors/V2DutchOrderReactor.sol";
...
contract ERC20ETHIntegrationTest is Test, DeployPermit2, PermitSignature, DelegationHandler {
    ...
    function test_V2DutchOrderWithNativeInput() public {
        ...
        DutchOutput[] memory outputs = OutputsBuilder.singleDutch(
            address(tokenOut),
            outputAmount,
            outputAmount,
        ...
        SignedOrder memory signedOrder = SignedOrder(
            abi.encode(order),
            signOrder(signerPrivateKey, address(permit2), order)
        );
    ...
    function test_PriorityOrderWithNativeInput() public {
        ...
        PriorityCosignerData memory cosignerData = PriorityCosignerData({
    ...
    function cosignV2Order(bytes32 orderHash, V2CosignerData memory cosignerData) private pure returns (bytes memory sig) {
    ...
    function cosignPriorityOrder(bytes32 orderHash, PriorityCosignerData memory cosignerData) private view returns (bytes memory sig) {
        bytes32 msgHash = keccak256(abi.encodePacked(orderHash, block.chainid, abi.encode(cosignerData)));
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(cosignerPrivateKey, msgHash);
        sig = bytes.concat(r, s, bytes1(v));
    }
