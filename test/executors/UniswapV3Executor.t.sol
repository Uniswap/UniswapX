...
contract UniswapV3ExecutorTest is Test, PermitSignature, GasSnapshot {
    ...
    function testExecuteBatch() public {
        ...
        Signature memory sig1 = signOrder(
            vm, makerPrivateKey, address(permitPost), order1.info, InputToken(order1.input.token, order1.input.endAmount), keccak256(abi.encode(order1))
        );
        ...
        Signature memory sig2 = signOrder(
            vm, makerPrivateKey, address(permitPost), order2.info, InputToken(order2.input.token, order2.input.endAmount), keccak256(abi.encode(order2))
        );
